...
import { Post, Category, Tag, BreadcrumbItem } from '@/types';
...
export default function Edit({ post, categories, tags }: Props) {
  const { data, setData, post: postRequest, processing, errors } = useForm<FormData>({
    title: post.title,
    ...
    meta: Object.fromEntries(post.postMeta?.map(m => [m.key, m.value]) || []),
  ...
  const [metaFields, setMetaFields] = useState<MetaField[]>(
    Object.entries(data.meta).map(([key, value]) => ({ key, value: String(value) }))
  );
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const metaObject = metaFields.reduce<Record<string, string>>((acc, { key, value }) => {
      if (key && value) {
        acc[key] = value;
      }
      return acc;
